      ...
      >
        ...
        <div className="flex justify-between items-start px-5 pt-5 pb-1">
          <div className="flex-1 flex flex-col">
            ...
            {/* ↓↓↓ 控制两者纵向间距的定义 (mt-6) ↓↓↓ */}
            <span className="font-bold text-[14px] ml-[8px] mb-2 inline-block text-gray-700 mt-2">
              V1.0-候鸟老人信息管理闭环
            </span>
            {/* ↑↑↑ 控制两者纵向间距的定义 (mt-6) ↑↑↑ */}
          </div>
        ...
        <div className="px-5 pb-4 pt-1 text-gray-700 text-sm leading-relaxed">
          <ol className="pl-4 space-y-1 list-decimal">
            <ol className="pl-4 space-y-1 list-decimal">
              ...
              <li>
                <b>全维档案管理</b>：支持候鸟老人信息的增/删/改/查
              </li>
              <li>
                <b>智能批量导入</b>：Excel一键上传,自动清洗格式极速入库
              </li>
              <li className="mb-4">
                <b>跨域自动流转</b>：上传数据即刻同步至对应迁入地社区
              </li>
...
export default function Home() {
  ...
  const [editingPerson, setEditingPerson] = useState<ElderlyPerson | null>(
    null,
  );
  ...
  const renderContent = () => {
    ...
    if (activeMenu === "migrate-in" || activeMenu === "migrate-out") {
      const title =
        activeMenu === "migrate-in" ? "迁入候鸟老人" : "迁出候鸟老人";
      return (
        <ElderlyTable
          title={title}
          onEdit={handleEdit}
